...
typedef NS_ENUM(NSUInteger, YNPasscodeStatus){
    /* The passcode status was unknown */
    YNPasscodeStatusUnknown   = 0,
    /* The passcode is enabled */
    YNPasscodeStatusEnabled   = 1,
    /* The passcode is disabled */
    YNPasscodeStatusDisabled  = 2
};

@interface UIDevice (YNPasscodeStatus)

/**
 *  Determines if the device supports the `passcodeStatus` check. Passcode check is only supported on iOS 8.
 */
@property (readonly) BOOL yn_passcodeStatusSupported;
 ...
 */
@property (readonly) YNPasscodeStatus yn_passcodeStatus;
