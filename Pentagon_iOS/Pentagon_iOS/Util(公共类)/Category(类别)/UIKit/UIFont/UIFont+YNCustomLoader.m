...
@implementation UIFont (YNCustomLoader)
 ...
 */
+ (BOOL) yn_deviceHasFullSupportForFontCollections {
 ...
 */
+ (BOOL)yn_registerFromURL:(NSURL *)fontURL {
...
/**
 ...
 @return Registration result
 */
+ (BOOL)yn_registerFromCGFont:(CGFontRef)fontRef {
...
+ (NSArray *)yn_registerFontFromURL:(NSURL *)fontURL {
    // Dictionary creation
    ...
    @synchronized(appRegisteredCustomFonts) {
        ...
        if (fontPSNames == nil) {
            ...
            // Check features
            if ([UIFont yn_deviceHasFullSupportForFontCollections]) {
                ...
                if (fontDescriptors) {
                    ...
                    if ([fontPSNames count] > 0) {
                        ...
                        if ([UIFont yn_registerFromURL:fontURL]) {
                    ...
                    if (loadedFont != NULL) {
                        ...
                        if ([singleFontValidExtensions containsObject:[fontURL pathExtension]]) {
                            ...
                            if ([UIFont fontWithName:fontPSNames[0] size:kSizePlaceholder] == nil) {
                                ...
                                // If registration went ok
                                if ([UIFont yn_registerFromCGFont:loadedFont]) {
                                    // Add url to this library
                                    [appRegisteredCustomFonts setObject:fontPSNames
                                                                 forKey:fontURL];
...
+ (UIFont *) yn_customFontWithURL:(NSURL *)fontURL size:(CGFloat)size {
    ...
    NSArray *fontPSNames = [UIFont yn_registerFontFromURL:fontURL];
...
+ (UIFont *) yn_customFontOfSize:(CGFloat)size withName:(NSString *)name withExtension:(NSString *)extension {
    // Get url for font resource
    NSURL *fontURL = [[[NSBundle mainBundle] URLForResource:name withExtension:extension] absoluteURL];
    
    return [UIFont yn_customFontWithURL:fontURL size:size];
}
