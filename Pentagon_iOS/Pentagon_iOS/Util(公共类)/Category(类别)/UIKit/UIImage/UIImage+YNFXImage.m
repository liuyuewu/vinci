...
@implementation UIImage (YNFXImage)
...
- (UIImage *)yn_imageScaledToFitSize:(CGSize)size
{
    ...
    {
        return [self yn_imageScaledToSize:CGSizeMake(size.width, size.width / aspect)];
    }
    else
    {
        return [self yn_imageScaledToSize:CGSizeMake(size.height * aspect, size.height)];
    }
}
...
- (UIImage *)yn_reflectedImageWithScale:(CGFloat)scale
{
	...
	//clip to gradient
	CGContextClipToMask(context, bounds, [[self class] yn_gradientMask]);
...
{
    //get reflected image
    UIImage *reflection = [self yn_reflectedImageWithScale:scale];
