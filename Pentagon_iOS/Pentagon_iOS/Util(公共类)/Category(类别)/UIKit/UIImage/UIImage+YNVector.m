...
@implementation UIImage(YNVector)
...
+ (UIImage *)yn_iconWithFont:(UIFont *)font named:(NSString *)iconNamed withTintColor:(UIColor *)tintColor clipToBounds:(BOOL)clipToBounds forSize:(CGFloat)fontSize{
    ...
    UIImage *image = [[self yn_cache] objectForKey:identifier];
    if(image == nil){
        ...
        if(!CGSizeEqualToSize(CGSizeZero, imageSize)){
            ...
            [[self yn_cache] setObject:image forKey:identifier];
        }
    }
    return image;
}

+ (UIImage *)yn_imageWithPDFNamed:(NSString *)pdfNamed forHeight:(CGFloat)height{
    return [self yn_imageWithPDFNamed:pdfNamed withTintColor:nil forHeight:height];
}

+ (UIImage *)yn_imageWithPDFNamed:(NSString *)pdfNamed withTintColor:(UIColor *)tintColor forHeight:(CGFloat)height{
    NSString *pdfFile = [[NSBundle mainBundle] pathForResource:pdfNamed ofType:@"pdf"];
    return [self yn_imageWithPDFFile:pdfFile withTintColor:tintColor forSize:CGSizeMake(MAXFLOAT, height)];
}

+ (UIImage *)yn_imageWithPDFFile:(NSString *)pdfFile withTintColor:(UIColor *)tintColor forSize:(CGSize)size{
