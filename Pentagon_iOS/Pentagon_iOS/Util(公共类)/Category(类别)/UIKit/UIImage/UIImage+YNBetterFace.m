...
@implementation UIImage (ynBetterFace)

- (UIImage *)yn_betterFaceImageForSize:(CGSize)size
                           accuracy:(YNAccuracy)accurary;
{
    NSArray *features = [UIImage _yn_faceFeaturesInImage:self accuracy:accurary];
    ...
    if ([features count]==0) {
        ...
        return [self _yn_subImageForFaceFeatures:features
                                         size:size];
    }
}

- (UIImage *)_yn_subImageForFaceFeatures:(NSArray *)faceFeatures size:(CGSize)size
{
...
+ (NSArray *)_yn_faceFeaturesInImage:(UIImage *)image accuracy:(YNAccuracy)accurary
{
    CIImage *ciImage = [CIImage imageWithCGImage:image.CGImage];
    NSString *accuraryStr = (accurary == YNAccuracyLow) ? CIDetectorAccuracyLow : CIDetectorAccuracyHigh;
