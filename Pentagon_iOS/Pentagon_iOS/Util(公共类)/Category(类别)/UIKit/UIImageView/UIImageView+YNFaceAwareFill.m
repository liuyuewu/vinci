...
static CIDetector * _yn_faceDetector;


@implementation UIImageView (YNFaceAwareFill)

+ (void)initialize
{
    _yn_faceDetector = [CIDetector detectorOfType:CIDetectorTypeFace
                                       context:nil
...
- (void)yn_faceAwareFill {
    // Safe check!
    if (self.image == nil) {
        return;
    }
    
    CGRect facesRect = [self yn_rectWithFaces];
    if (facesRect.size.height + facesRect.size.width == 0)
        return;
    self.contentMode = UIViewContentModeTopLeft;
    [self yn_scaleImageFocusingOnRect:facesRect];
}

- (CGRect) yn_rectWithFaces {
    // Get a CIIImage
    ...
    CIDetector* detector = _yn_faceDetector;
...
- (void) yn_scaleImageFocusingOnRect:(CGRect) facesRect {
