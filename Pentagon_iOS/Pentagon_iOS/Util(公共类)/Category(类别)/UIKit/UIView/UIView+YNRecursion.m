//
//  UIView+Recursion.m
//  ynCategories
//
//  Created by Jakey on 15/6/23.
//  Copyright © 2015年 www.skyfox.org. All rights reserved.
//

#import "UIView+YNRecursion.h"

@implementation UIView (YNRecursion)
/**
 *  @brief  寻找子视图
 *
 *  @param recurse 回调
 *
 *  @return  Return YES from the block to recurse into the subview.
 Set stop to YES to return the subview.
 */
- (UIView*)yn_findViewRecursively:(BOOL(^)(UIView* subview, BOOL* stop))recurse
{
    for( UIView* subview in self.subviews ) {
        BOOL stop = NO;
        if( recurse( subview, &stop ) ) {
            return [subview yn_findViewRecursively:recurse];
        } else if( stop ) {
            return subview;
        }
    }
    
    return nil;
}


-(void)yn_runBlockOnAllSubviews:(YNSubviewBlock)block
{
    block(self);
    for (UIView* view in [self subviews])
    {
        [view yn_runBlockOnAllSubviews:block];
    }
}

-(void)yn_runBlockOnAllSuperviews:(YNSuperviewBlock)block
{
    block(self);
    if (self.superview)
    {
        [self.superview yn_runBlockOnAllSuperviews:block];
    }
}

-(void)yn_enableAllControlsInViewHierarchy
{
    [self yn_runBlockOnAllSubviews:^(UIView *view) {
        
        if ([view isKindOfClass:[UIControl class]])
        {
            [(UIControl *)view setEnabled:YES];
        }
        else if ([view isKindOfClass:[UITextView class]])
        {
            [(UITextView *)view setEditable:YES];
        }
    }];
}

-(void)yn_disableAllControlsInViewHierarchy
{
    [self yn_runBlockOnAllSubviews:^(UIView *view) {
        
        if ([view isKindOfClass:[UIControl class]])
        {
            [(UIControl *)view setEnabled:NO];
        }
        else if ([view isKindOfClass:[UITextView class]])
        {
            [(UITextView *)view setEditable:NO];
        }
    }];
}
@end
