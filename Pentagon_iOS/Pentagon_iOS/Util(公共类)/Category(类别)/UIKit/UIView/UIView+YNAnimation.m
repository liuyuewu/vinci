...
@implementation UIView (ynAnimation)

#pragma mark - Moves

- (void)yn_moveTo:(CGPoint)destination duration:(float)secs option:(UIViewAnimationOptions)option {
    [self yn_moveTo:destination duration:secs option:option delegate:nil callback:nil];
}
...
- (void)yn_raceTo:(CGPoint)destination withSnapBack:(BOOL)withSnapBack {
    [self yn_raceTo:destination withSnapBack:withSnapBack delegate:nil callback:nil];
}
...
- (void)yn_rotate:(int)degrees secs:(float)secs delegate:(id)delegate callback:(SEL)method {
    [UIView animateWithDuration:secs 
                        ...
                        options:UIViewAnimationOptionCurveLinear
                     animations:^{
                         self.transform = CGAffineTransformRotate(self.transform, yn_radiansForDegrees(degrees));
                     }
                     completion:^(BOOL finished) { 
                         if (delegate != nil) {
#pragma clang diagnostic push
...
- (void)yn_spinClockwise:(float)secs {
    [UIView animateWithDuration:secs/4 
                        ...
                        options:UIViewAnimationOptionCurveLinear
                     animations:^{
                         self.transform = CGAffineTransformRotate(self.transform, yn_radiansForDegrees(90));
                     }
                     completion:^(BOOL finished) { 
                         [self yn_spinClockwise:secs];
                     }];
}
...
- (void)yn_spinCounterClockwise:(float)secs {
    [UIView animateWithDuration:secs/4 
                        ...
                        options:UIViewAnimationOptionCurveLinear
                     animations:^{
                         self.transform = CGAffineTransformRotate(self.transform, yn_radiansForDegrees(270));
                     }
                     completion:^(BOOL finished) { 
                         [self yn_spinCounterClockwise:secs];
                     }];
}
...
- (void)yn_drainAway:(float)secs {
    self.tag = 20;
	[NSTimer scheduledTimerWithTimeInterval:secs/50 target:self selector:@selector(yn_drainTimer:) userInfo:nil repeats:YES];
}

- (void)yn_drainTimer:(NSTimer*)timer {
    ...
    [UIView animateWithDuration:secs/2 
                     ...
                     completion:^(BOOL finished) { 
                         [UIView animateWithDuration:secs/2 
                                          ...
                                          completion:^(BOOL finished) { 
                                              if (continuously) {
                                                  [self yn_pulse:secs continuously:continuously];
                                              }
                                          }];
                     }];
}
