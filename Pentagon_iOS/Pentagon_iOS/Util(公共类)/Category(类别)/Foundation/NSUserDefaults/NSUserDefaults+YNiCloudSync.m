...
@implementation NSUserDefaults (YNiCloudSync)
...
- (void)yn_removeValueForKey:(NSString *)key iCloudSync:(BOOL)sync
{
    [self yn_removeObjectForKey:key iCloudSync:sync];
}



-(void)yn_setObject:(id)value forKey:(NSString *)defaultName iCloudSync:(BOOL)sync
{
    if (sync)
...
- (void)yn_removeObjectForKey:(NSString *)key iCloudSync:(BOOL)sync
{
    if (sync)
