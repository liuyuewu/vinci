...
static NSString *const YNAutocodingException = @"YNAutocodingException";

@implementation NSObject (YNAutoCoding)

+ (BOOL)supportsSecureCoding
{
    return YES;
}
+ (instancetype)yn_objectWithContentsOfFile:(NSString *)filePath
{
    //load the file
...
- (NSDictionary *)yn_codableProperties
{
    ...
    if (!codableProperties)
    {
        ...
        {
            [(NSMutableDictionary *)codableProperties addEntriesFromDictionary:[subclass yn_codableProperties]];
...
- (NSDictionary *)yn_dictionaryRepresentation
{
    ...
    for (__unsafe_unretained NSString *key in [self yn_codableProperties])
    {
...
- (void)yn_setWithCoder:(NSCoder *)aDecoder
{
    ...
    NSDictionary *properties = [self yn_codableProperties];
    for (NSString *key in properties)
    {
        ...
        if (object)
        {
            ...
            {
                [NSException raise:YNAutocodingException format:@"Expected '%@' to be a %@, but was actually a %@", key, propertyClass, [object class]];
            }
...
{
    [self yn_setWithCoder:aDecoder];
    return self;
}
#pragma clang diagnostic pop
- (void)encodeWithCoder:(NSCoder *)aCoder
{
    for (NSString *key in [self yn_codableProperties])
    {
