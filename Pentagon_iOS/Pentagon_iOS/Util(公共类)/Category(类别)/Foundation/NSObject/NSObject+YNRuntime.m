...
BOOL yn_method_swizzle(Class klass, SEL origSel, SEL altSel)
{
    if (!klass)
        return NO;
...
void yn_method_append(Class toClass, Class fromClass, SEL selector)
{
...
void yn_method_replace(Class toClass, Class fromClass, SEL selector)
{
    if (!toClass || !fromClass || ! selector)
        return;
...
@implementation NSObject (YNRuntime)
+ (void)yn_swizzleMethod:(SEL)originalMethod withMethod:(SEL)newMethod
{
    yn_method_swizzle(self.class, originalMethod, newMethod);
}

+ (void)yn_appendMethod:(SEL)newMethod fromClass:(Class)klass
{
    yn_method_append(self.class, klass, newMethod);
}

+ (void)yn_replaceMethod:(SEL)method fromClass:(Class)klass
{
    yn_method_replace(self.class, klass, method);
}

- (BOOL)yn_respondsToSelector:(SEL)selector untilClass:(Class)stopClass
{
    return [self.class yn_instancesRespondToSelector:selector untilClass:stopClass];
}

- (BOOL)yn_superRespondsToSelector:(SEL)selector
{
    return [self.superclass instancesRespondToSelector:selector];
}

- (BOOL)yn_superRespondsToSelector:(SEL)selector untilClass:(Class)stopClass
{
    return [self.superclass yn_instancesRespondToSelector:selector untilClass:stopClass];
}

+ (BOOL)yn_instancesRespondToSelector:(SEL)selector untilClass:(Class)stopClass
{
