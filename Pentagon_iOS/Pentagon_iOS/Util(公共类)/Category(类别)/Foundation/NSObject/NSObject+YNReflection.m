...
@implementation NSObject (YNReflection)
...
{
    return [[self class] yn_propertyKeys];
}
+ (NSArray *)yn_propertyKeys {
...
{
    return [[self class] yn_propertiesInfo];
}
/**
...
+ (NSArray *)yn_propertiesInfo
{
    ...
    for (int i = 0; i < propertyCount; i++)
    {
        [propertieArray addObject:({
            ...
            NSDictionary *dictionary = [self yn_dictionaryWithProperty:properties[i]];
...
+ (NSArray *)yn_propertiesWithCodeFormat
{
    ...
    NSArray *properties = [[self class] yn_propertiesInfo];
...
-(NSArray*)yn_methodListInfo{
    ...
    for (int i = 0; i < count ; i++)
    {
        ...
        for (int index=0; index<argumentsCount; index++) {
            ...
            [arguments addObject:[[self class] yn_decodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] yn_decodeType:returnType];
        NSString *encodeString = [[self class] yn_decodeType:encoding];
...
-(NSDictionary *)yn_protocolList{
    return [[self class]yn_protocolList];
}
+ (NSDictionary *)yn_protocolList
{
...
+ (NSArray *)yn_instanceVariable
{
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] yn_decodeType:ivar_getTypeEncoding(ivars[i])];
...
#pragma mark -- help
+ (NSDictionary *)yn_dictionaryWithProperty:(objc_property_t)property
{
...
+ (NSString *)yn_decodeType:(const char *)cString
{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString yn_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
