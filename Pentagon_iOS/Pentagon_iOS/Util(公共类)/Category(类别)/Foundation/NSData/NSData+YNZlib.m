...
NSString *const ynZlibErrorDomain = @"se.bitba.ynZlibErrorDomain";
NSString *const ynZlibErrorInfoKey = @"zerror";

@implementation NSData (YNZlib)

static void *yn_zlibOpen()
{
    static void *libz;
...
- (NSData *)yn_dataByInflatingWithError:(NSError *__autoreleasing *)error
{
    ...
    [self yn_inflate:^(NSData *toAppend) {
...
- (NSData *)yn_dataByDeflatingWithError:(NSError *__autoreleasing *)error
{
    if (![self length]) return [self copy];
    NSMutableData *outData = [NSMutableData data];
    [self yn_deflate:^(NSData *toAppend) {
        [outData appendData:toAppend];
    }
               error:error];
    return outData;
...
- (BOOL)yn_inflate:(void (^)(NSData *))processBlock
             error:(NSError *__autoreleasing *)error
{
    void *libz = yn_zlibOpen();
    ...
    if (ret != Z_OK) {
        if (error) *error = [NSError errorWithDomain:ynZlibErrorDomain
                                                code:ynZlibErrorCodeInflationError
                                            userInfo:@{ynZlibErrorInfoKey : @(ret)}];
        return NO;
    }
...
// Adapted from http://www.zlib.net/zpipe.c
- (BOOL)yn_deflate:(void (^)(NSData *))processBlock
             error:(NSError *__autoreleasing *)error
{
    void *libz = yn_zlibOpen();
    //    int (*deflateInit2_)(z_streamp, int, int, int, int, int, const char *, int) =
    ...
    if (ret != Z_OK) {
        if (error) *error = [NSError errorWithDomain:ynZlibErrorDomain
                                                code:ynZlibErrorCodeDeflationError
                                            userInfo:@{ynZlibErrorInfoKey : @(ret)}];
        return NO;
    }
    Bytef *source = (Bytef *)[self bytes]; // yay
...
- (BOOL)yn_writeDeflatedToFile:(NSString *)path
                         error:(NSError *__autoreleasing *)error
{
    NSFileHandle *f = yn_createOrOpenFileAtPath(path, error);
    if (!f) return NO;
    BOOL success = YES;
    if ([self length]) {
        success = [self yn_deflate:
                   ^(NSData *toAppend) {
...
static NSFileHandle *yn_createOrOpenFileAtPath(NSString *path, NSError *__autoreleasing *error)
{
    if (![[NSFileManager defaultManager] fileExistsAtPath:path]) {
        ...
        if (!success) {
            if (error) *error = [NSError errorWithDomain:ynZlibErrorDomain
                                                    code:ynZlibErrorCodeCouldNotCreateFileError
                                                userInfo:nil];
            return nil;
        }
    }
