...
@implementation NSDate (YNInternetDateTime)

// Instantiate single date formatter
+ (NSDateFormatter *)yn_internetDateTimeFormatter {
    @synchronized(self) {
...
+ (NSDate *)yn_dateFromInternetDateTimeString:(NSString *)dateString formatHint:(DateFormatHint)hint {
    // Keep dateString around a while (for thread-safety)
    NSDate *date = nil;
    if (dateString) {
        if (hint != DateFormatHintRFC3339) {
            // Try RFC822 first
            date = [NSDate yn_dateFromRFC822String:dateString];
            if (!date) date = [NSDate yn_dateFromRFC3339String:dateString];
        } else {
            // Try RFC3339 first
            date = [NSDate yn_dateFromRFC3339String:dateString];
            if (!date) date = [NSDate yn_dateFromRFC822String:dateString];
        }
    }
...
+ (NSDate *)yn_dateFromRFC822String:(NSString *)dateString {
    ...
    NSDate *date = nil;
    if (dateString) {
        NSDateFormatter *dateFormatter = [NSDate yn_internetDateTimeFormatter];
