...
@implementation NSInvocation (YNBb)
...
+ (id)yn_doInstanceMethodTarget:(id)target selectorName:(NSString *)selectorName args:(NSArray *)args
{
    ...
    [invocation yn_setArgumentsWithArray:args];
...
- (void)yn_setArgumentsWithArray:(NSArray *)array
{
    if (nil != array && array.count > 0) {
        ...
        if (array.count != numargs) {
            ...
            [self yn_setArgumentWithObject:arg atIndex:0];
        }else{
            for (NSUInteger i = 0; i < numargs; i++) {
                [self yn_setArgumentWithObject:array[i] atIndex:i];
            }
        }
    }
}
