...
#import "CLLocation+YNCH1903.h"


@implementation CLLocation (YNCH1903)


- (id) initWithCH1903x:(double)x y:(double)y
{
    self = [self initWithLatitude:[CLLocation yn_CHtoWGSlatWithX:x y:y] longitude:[CLLocation yn_CHtoWGSlongWithX:x y:y]];
    if (self != nil) {
...
- (double)yn_CH1903Y {
    return [CLLocation yn_WGStoCHyWithLatitude:self.coordinate.latitude longitude:self.coordinate.longitude];
}

- (double)yn_CH1903X {
    return [CLLocation yn_WGStoCHxWithLatitude:self.coordinate.latitude longitude:self.coordinate.longitude];
}


+ (double)yn_CHtoWGSlatWithX:(double)x y:(double)y {
...
+ (double)yn_CHtoWGSlongWithX:(double)x y:(double)y {
...
+ (double)yn_WGStoCHyWithLatitude:(double)lat longitude:(double)lng {
    // Converts degrees dec to sex
    lat = [CLLocation yn_decToSex:lat];
    lng = [CLLocation yn_decToSex:lng];
    
    // Converts degrees to seconds (sex)
    lat = [CLLocation yn_degToSec:lat];
    lng = [CLLocation yn_degToSec:lng];
...
+ (double)yn_WGStoCHxWithLatitude:(double)lat longitude:(double)lng {
    // Converts degrees dec to sex
    lat = [CLLocation yn_decToSex:lat];
    lng = [CLLocation yn_decToSex:lng];
...
+ (double)yn_decToSex:(double)angle {
    // Extract DMS
    int deg = (int) angle;
...
+ (double)yn_degToSec:(double)angle {
    // Extract DMS
    int deg = (int) angle;
...
+ (double)yn_sexToDec:(double)angle {
    // Extract DMS
    int deg = (int) angle;
    int min = (int) ((angle-deg)*100);
    double sec = (((angle-deg)*100.0) - min) * 100.0;
